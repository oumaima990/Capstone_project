...
from knowledge.models import KnowledgeComponent, KnowledgeNode, Grade,Unit,UnitProgress
...
class StudentViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            print(f"Teacher ID: {teacher.user.id}, Username: {teacher.user.username}")
            ...
            #automatically intilize the 
            grades = Grade.objects.all().order_by("order")
            for grade in grades:
                for unit in grade.units.all().order_by("order"):
                    UnitProgress.objects.create(
                        student=student,
                        unit=unit,
                        unlocked=(grade.id == grades.first().id and unit.order == 1)  # Unlock the first unit of the first grade
                    )
    ...
    def get_all_students_teacher_based(self, request):
        ...
        students = Student.objects.filter(teacher__user__id=teacher_id).select_related('user', 'teacher__user')
