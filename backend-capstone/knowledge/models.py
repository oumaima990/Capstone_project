...
class Grade(models.Model):
    ...
    order = models.IntegerField()  # Order of the unit within the grade
...
class Unit(models.Model):
    ...
    title = models.CharField(max_length=100)  # e.g., "Unit 1", "Unit 2"
    order = models.IntegerField()  # Order of the unit within the grade
...
class UnitProgress(models.Model):
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name="unit_progress")
    unit = models.ForeignKey(Unit, on_delete=models.CASCADE, related_name="progress")
    unlocked = models.BooleanField(default=False)
   
    def __str__(self):
        return f"{self.student.username} - {self.unit.title} - Unlocked: {self.unlocked}"
class QuizAttempt(models.Model):
    student = models.ForeignKey(Student, on_delete=models.CASCADE, related_name="quiz_attempts")
    unit = models.ForeignKey(Unit, on_delete=models.CASCADE, related_name="quiz_attempts")
    score = models.FloatField()  # Percentage score of the quiz
    passed = models.BooleanField()  # Whether the student passed the quiz
    attempt_date = models.DateTimeField(auto_now_add=True)  # Timestamp of the attempt
 
    def __str__(self):
        return f"{self.student.user.username} - {self.unit.title} - Score: {self.score} - Passed: {self.passed}"
    
    
class QuizResponse(models.Model):
    quiz_attempt = models.ForeignKey(QuizAttempt, on_delete=models.CASCADE, related_name="responses")
    question = models.ForeignKey(Question, on_delete=models.CASCADE, related_name="responses")
    student_answer = models.TextField()  # The answer provided by the student
    is_correct = models.BooleanField()  # Whether the answer was correct
 
    def __str__(self):
        return f"Attempt: {self.quiz_attempt.id} - Question: {self.question.id} - Correct: {self.is_correct}"